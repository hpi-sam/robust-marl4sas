...
/**
 * Injects failures into the model, however, the number of injected failures is
 * below the threshold for a CF2. Hence, it injects a {@link IssueType#NO_CF2}.
 * 
 * @author thomas vogel
 * @version 0.30
 *
 */
public class NoCF2_Injector implements Injector<ProvidedInterface> {

	private final static Logger LOGGER = Logger.getLogger(NoCF2_Injector.class
			.getName());
	 ...
	 *            will inject less failures than the threshold.
	 */
	...
	public String getName() {
		return "Change (< #" + this.failureThreshold + " Failures) Injector";
	}
	...
	public String getDescription() {
		return "Adds less than " + this.failureThreshold
	...
	public boolean inject(ProvidedInterface providedInterface) {
		...
		if (component.getState() == ComponentLifeCycle.STARTED) {
			...
			if (isUsed) {
				int numbersOfFailures = SimulatorUtil.randInt(1,
				...
				LOGGER.info(" -- Attached " + numbersOfFailures
